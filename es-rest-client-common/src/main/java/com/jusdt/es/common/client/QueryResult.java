...
public class QueryResult {
	...
	protected static final ImmutableList<MetaField> META_FIELDS = ImmutableList.of(
			new MetaField(ES_METADATA_ID, "_id", ESClientId.class),
			new MetaField(ES_METADATA_VERSION, "_version", ESClientVersion.class));
	...
	public QueryResult(QueryResult source) {
	...
	protected List<JsonElement> extractSource(boolean addEsMetadataFields) {
		List<JsonElement> sourceList = new ArrayList<>();
								...
								if (source != null) {
									...
									if (addEsMetadataFields) {
										for (MetaField metaField : META_FIELDS) {
											copy.add(metaField.internalFieldName,
													currentObj.get(metaField.esFieldName));
										}
									}
									sourceList.add(copy);
								}
		...
		try {
			...
			while (clazz != null && !allFieldsFound) {
				...
				for (Field field : fields) {
					...
					for (MetaField metaField : META_FIELDS) {
						if (field.isAnnotationPresent(metaField.annotationClass)
								&& setAnnotatedField(obj, source, field, metaField.internalFieldName)) {
							foundFieldsCount++;
						}
					}
				}
			...
			log.error("Unhandled exception occurred while converting source to the object ." + type.getCanonicalName(),
					e);
		}
	...
	public String toString() {
		return "Result: " + getJsonString() + ", isSucceeded: " + isSucceeded() + ", response code: "
				+ getResponseCode() + ", error message: " + getErrorMessage();
	}
