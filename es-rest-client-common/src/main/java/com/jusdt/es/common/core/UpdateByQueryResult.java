...
public class UpdateByQueryResult extends QueryResult {
	...
	public long getConflictsCount() {
		return (jsonObject != null && jsonObject.has("version_conflicts"))
				? jsonObject.get("version_conflicts").getAsLong() : 0L;
	}
	...
	public int getBulkRetryCount() {
		return (jsonObject != null && jsonObject.has("retries") && jsonObject.getAsJsonObject("retries").has("bulk"))
				? jsonObject.getAsJsonObject("retries").get("bulk").getAsInt() : 0;
	}

	public int getSearchRetryCount() {
		return (jsonObject != null && jsonObject.has("retries") && jsonObject.getAsJsonObject("retries").has("search"))
				? jsonObject.getAsJsonObject("retries").get("search").getAsInt() : 0;
	}
	...
	public String toString() {
		return "Updated: " + getUpdatedCount() + ", conflicts: " + getConflictsCount() + ", time taken: "
				+ getMillisTaken() + ", did time out: " + didTimeOut() + ", batches: " + getBatchCount() + ", retries: "
				+ getRetryCount() + ", bulk retries: " + getBulkRetryCount() + ", search retries: "
				+ getSearchRetryCount() + ", noops: " + getNoopCount();
	}
