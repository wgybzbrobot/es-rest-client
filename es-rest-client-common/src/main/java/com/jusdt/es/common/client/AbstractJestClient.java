...
public abstract class AbstractJestClient implements JestClient {
	...
	private final AtomicReference<ServerPool> serverPoolReference = new AtomicReference<ServerPool>(
			new ServerPool(ImmutableSet.<String> of()));
	...
	public void setServers(Set<String> servers) {
		...
		if (log.isInfoEnabled()) {
			log.info("Setting server pool to a list of {} servers: [{}]", servers.size(),
					Joiner.on(',').join(scrubServerURIs(servers)));
		}
	...
	Set<String> scrubServerURIs(Set<String> servers) {
		...
		for (String server : servers) {
			...
			try {
				final URI scrubbedURI = new URI(originalURI.getScheme(), null, // Remove user info
						originalURI.getHost(), originalURI.getPort(), originalURI.getPath(), originalURI.getQuery(),
	...
	/**
	 * @throws com.jusdt.es.common.client.config.NoServerConfiguredException
	 */
