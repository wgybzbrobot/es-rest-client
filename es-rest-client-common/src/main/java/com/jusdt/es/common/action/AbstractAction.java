...
public abstract class AbstractAction<T extends QueryResult> implements Action<T> {
	...
	protected T createNewElasticSearchResult(T result, String responseBody, int statusCode, String reasonPhrase,
			Gson gson) {
	...
	public static String getIdFromSource(Object source) {
		...
		for (Field field : fields) {
			if (field.isAnnotationPresent(ESClientId.class)) {
				try {
	...
	protected String buildQueryString() throws UnsupportedEncodingException {
		...
		for (Map.Entry<String, Object> entry : parameterMap.entries()) {
			queryString.append(URLEncoder.encode(entry.getKey(), CHARSET)).append("=")
					.append(URLEncoder.encode(entry.getValue().toString(), CHARSET)).append("&");
		}
	...
	public String toString() {
		return MoreObjects.toStringHelper(this).add("uri", getURI(ElasticSearchVersion.UNKNOWN))
				.add("method", getRestMethodName()).toString();
	}
	...
	public boolean equals(Object obj) {
		...
		return Objects.equals(getURI(ElasticSearchVersion.UNKNOWN), rhs.getURI(ElasticSearchVersion.UNKNOWN))
				...
				&& Objects.equals(getHeaders(), rhs.getHeaders()) && Objects.equals(payload, rhs.payload);
	}
