...
public class NodeChecker extends AbstractScheduledService {
	...
	private final static Pattern INETSOCKETADDRESS_PATTERN = Pattern
			.compile("(?:inet\\[)?(?:(?:[^:]+)?\\/)?([^:]+):(\\d+)\\]?");
	...
	public NodeChecker(ESClient jestClient, ClientConfig clientConfig) {
		...
		this.scheduler = Scheduler.newFixedDelaySchedule(0l, clientConfig.getDiscoveryFrequency(),
				...
				for (Entry<String, JsonElement> entry : nodes.entrySet()) {
					...
					if (host.has("version")) {
						int majorVersion = Integer
								.parseInt(Splitter.on('.').splitToList(host.get("version").getAsString()).get(0));
	...
	protected ScheduledExecutorService executor() {
		final ScheduledExecutorService executor = Executors.newSingleThreadScheduledExecutor(
				new ThreadFactoryBuilder().setDaemon(true).setNameFormat(serviceName()).build());
