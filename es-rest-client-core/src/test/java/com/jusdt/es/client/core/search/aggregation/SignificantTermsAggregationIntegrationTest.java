...
import com.jusdt.es.common.core.search.aggregation.SignificantTermsAggregation;
...
public class SignificantTermsAggregationIntegrationTest extends AbstractIntegrationTest {
	...
	@Test
	public void testGetSignificantTermsAggregation() throws Exception {
		createIndex(INDEX);
		AcknowledgedResponse putMappingResponse = client().admin().indices()
				.putMapping(new PutMappingRequest(INDEX).type(TYPE)
		...
		SignificantTermsAggregation significantTerms = result.getAggregations()
				.getSignificantTermsAggregation("significant_terms1");
		...
		Aggregation aggregation = result.getAggregations().getAggregation("significant_terms1",
				SignificantTermsAggregation.class);
	...
	@Test
	public void testBadAggregationQueryResult() throws Exception {
		createIndex(INDEX);
		AcknowledgedResponse putMappingResponse = client().admin().indices()
				.putMapping(new PutMappingRequest(INDEX).type(TYPE)
