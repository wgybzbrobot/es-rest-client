...
import com.jusdt.es.common.core.search.aggregation.PercentileRanksAggregation;
...
public class PercentileRanksAggregationIntegrationTest extends AbstractIntegrationTest {
	...
	@Test
	public void testGetPercentileRanksAggregation() throws IOException {
		createIndex(INDEX);
		AcknowledgedResponse putMappingResponse = client().admin().indices()
				.putMapping(new PutMappingRequest(INDEX).type(TYPE).source(
						"{\"document\":{\"properties\":{\"response_millis\":{\"store\":true,\"type\":\"double\"}}}}",
						XContentType.JSON))
				.actionGet();
