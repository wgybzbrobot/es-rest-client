...
public class JestClientFactory {
	...
	public JestClient getObject() {
		...
		if (httpClientConfig.getMaxConnectionIdleTime() > 0) {
			...
			IdleConnectionReaper reaper = new IdleConnectionReaper(httpClientConfig,
	...
	private CloseableHttpClient createHttpClient(HttpClientConnectionManager connectionManager) {
		return configureHttpClient(
				HttpClients.custom().setConnectionManager(connectionManager).setDefaultRequestConfig(getRequestConfig())
						.setProxyAuthenticationStrategy(httpClientConfig.getProxyAuthenticationStrategy())
						.setRoutePlanner(getRoutePlanner())
						.setDefaultCredentialsProvider(httpClientConfig.getCredentialsProvider())).build();
	}

	private CloseableHttpAsyncClient createAsyncHttpClient(NHttpClientConnectionManager connectionManager) {
		return configureHttpClient(HttpAsyncClients.custom().setConnectionManager(connectionManager)
	...
	protected RequestConfig getRequestConfig() {
		return RequestConfig.custom().setConnectTimeout(httpClientConfig.getConnTimeout())
	...
	protected NHttpClientConnectionManager getAsyncConnectionManager() {
		...
		IOReactorConfig ioReactorConfig = IOReactorConfig.custom().setConnectTimeout(httpClientConfig.getConnTimeout())
	...
	protected HttpClientConnectionManager getConnectionManager() {
		...
		Registry<ConnectionSocketFactory> registry = RegistryBuilder.<ConnectionSocketFactory> create()
				...
				.register("https", httpClientConfig.getSslSocketFactory()).build();
